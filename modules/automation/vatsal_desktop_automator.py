...
class BOIAutomator:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        if not api_key:
            raise ValueError(
                "GEMINI_API_KEY required for command understanding")
...
5. Keep actions simple and atomic"""
    ...
    def execute_action(self, action: Dict) -> Tuple[bool, str]:
        ...
        try:
            if action_type == "open_app":
                ...
                return self._move_file(params.get("source"),
                                       params.get("dest"))
    ...
    def _take_screenshot(self, path: str) -> Tuple[bool, str]:
        ...
        os.makedirs(
            os.path.dirname(path) if os.path.dirname(path) else "screenshots",
            exist_ok=True)
...
Battery: {report['battery'].get('percent', 'N/A')}%"""
    ...
    def _shutdown_system(self, delay_seconds: int = 10) -> Tuple[bool, str]:
        ...
        try:
            ...
            if os_type == "Windows":
                ...
                subprocess.Popen(f'sudo shutdown -h +{delay_seconds//60}',
                                 shell=True)
                return True, f"⚠️ Computer will shutdown in {delay_seconds} seconds"
            elif os_type == "Linux":
                subprocess.Popen(f'sudo shutdown -h +{delay_seconds//60}',
    ...
    def _restart_system(self, delay_seconds: int = 10) -> Tuple[bool, str]:
        ...
        try:
            ...
            if os_type == "Windows":
                ...
                subprocess.Popen(f'sudo shutdown -r +{delay_seconds//60}',
                                 shell=True)
                return True, f"🔄 Computer will restart in {delay_seconds} seconds"
            elif os_type == "Linux":
                subprocess.Popen(f'sudo shutdown -r +{delay_seconds//60}',
    ...
    def execute_command(self,
                        user_input: str,
                        confirmation_callback=None) -> str:
